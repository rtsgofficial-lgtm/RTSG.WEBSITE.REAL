...
- `TURNSTILE_SECRET`: Cloudflare Turnstile widget secret used by the backend to verify tokens
- `TURNSTILE_HOSTNAMES`: Comma-separated production hostnames allowed by Turnstile siteverify, for example `rtsg.org,www.rtsg.org,news.rtsg.org`
- `VITE_TURNSTILE_SITE_KEY`: Optional frontend override for the Turnstile site key; defaults to the RTSG widget site key
